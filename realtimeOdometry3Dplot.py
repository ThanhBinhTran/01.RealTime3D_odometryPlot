...
for i in range (len(dx)):
    ...
    # update pose data
    drone.update_pose(dx[i], dy[i], dz[i], dx[i],dy[i],dz[i])
    platform.update_pose(px[i], py[i], pz[i], 0)
    
    ''' 3D plot'''
    # draw drone hardware and platform hardware and theirs tracjectory
    drone.plot_devices(ax_3D)
    drone.plot_tracjectory(ax_3D) 
    platform.plot_devices(ax_3D)
